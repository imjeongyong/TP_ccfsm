...
<body>
	...
	<div class="container">
		...
		<article>
			<h1 class="display-6 m-2 p-2 bg-danger-subtle" style="text-align: center;">레시피 게시판</h1>
			<!-- 검색 폼 -->
										...
										<tr>
											...
											<td class="text-center">
												<!-- 첨부 파일 --> <c:if test="${ not empty row.ofile }">
													<a class="BoardFile"
														href="../recipeboard/download.do?ofile=${ row.ofile }&sfile=${ row.sfile }&idx=${ row.idx }">[식단]</a>
												</c:if>
											</td>
										</tr>
									</c:forEach>
					...
					<div>
						...
						<c:if test="${sessionScope.UserId eq isAdmin }">
							<button type="button" class="btn btn-warning"
								onclick="location.href='../recipeboard/write.do';">글쓰기</button>
						</c:if>
					</div>
				</div>
			</div>
