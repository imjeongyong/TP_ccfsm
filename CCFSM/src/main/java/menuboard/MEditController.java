...
public class MEditController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/MenuBoard/Edit.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (result == 1) {
			// 수정 성공
			response.sendRedirect("../menuboard/view.do?idx=" + idx);
		} else {
			// 수정 실패
			JSFunction.alertLocation(response, "게시물 수정 실패", "../menuboard/view.do?idx=" + idx);
		}
	}
}